...
Four routes, each a self-contained `RequirementSet` (data, with verified thresholds). The **corpus** (`data/corpus/`, generated by `scripts/generate_corpus.py`) exercises each route with **20 cases / 106 documents** across six scenario families — used to backtest the whole workflow.
...
- **Gateway** (`gateway/`) — the channel-agnostic loop. Stateful per conversation: email state is keyed by the root `Message-ID`, so two threads from the same sender remain separate. It infers the route from newly authored text (quoted history is removed), accumulates documents within that thread, runs the supervisor, and replies with complete gap feedback. WhatsApp webhook (HMAC signature verification, fail-closed) and email IMAP polling both use the same loop.
- **Question boundaries** (`scripts/e2e_email_questions.py`) — live Gmail coverage for greetings, capability questions, route ambiguity/correction, multilingual input, unrelated questions, non-UK and unsupported routes, legal guarantees, hostile text, prompt injection, quoted history, and attachment-only email.
...
| Question/capability handling and warm scope boundaries | ✅ 27/27 live Gmail edge flows |
| Per-thread isolation and quoted-history removal | ✅ exact `Message-ID` / `References` E2E checks |
