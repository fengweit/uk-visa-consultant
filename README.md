...
| Scenario | Verdict | Example case |
|---|---|---|
| `complete` | `READY` | `student_001_complete` |
| missing a mandatory doc | `MISSING` | `student_002_missing_cas` |
| funds below threshold / 28-day dip | `INVALID` | `student_003_funds_insufficient`, `student_004_funds_28day_violation` |
| name mismatch across docs | `INCONSISTENT` | `student_005_name_mismatch` |
| passport expiring mid-stay | `EXPIRING` | `visitor_004_passport_expiring` |
| image-only (scanned) document | `INVALID` (needs OCR) | `visitor_005_scanned_passport` |

Full case list: `visitor_{001..005}`, `student_{001..005}`, `worker_{001..005}`, `spouse_{001..005}` — see each case's `case.json` for the ground-truth `expected` outcome.
